...
// from clicking 结束答辩, the backend may still be finalizing the round + the
...
export default function ReportPage() {
  ...
  const [loadingMsg, setLoadingMsg] = useState('答辩结果生成中，请稍候…');
  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      try {
        ...
        if (!terminal) {
          setLoadingMsg('答辩结束中，正在总结本轮表现…');
        } else {
          setLoadingMsg('答辩结果生成中，正在撰写评价报告…');
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-lg shadow p-4 flex items-center justify-between">
        <h1 className="text-xl font-bold">答辩报告 · #{id}</h1>
        {pdfUrl && (
